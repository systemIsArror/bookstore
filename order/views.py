...
def order_pay(request):
	...
	alipay = AliPay(
		...
		alipay_public_key_path=os.path.join(settings.BASE_DIR, "order/alipay_public_key.pem"), #支付宝的公钥，验证支付宝回传信息使用，不是总价的公钥
		sign_type="RSA2", #RSA或者RSA2
		debug=True, #默认Ｆａｌｓｅ
	)

	#电脑网站支付，需要跳转到https://openapi.alipaydev.com/gateway.do?+order_string
	total_pay = order.total_price + order.transit_price #精度
...
def check_pay(request):
	...
	alipay = AliPay(
		...
		alipay_public_key_path=os.path.join(settings.BASE_DIR, "df_order/alipay_public_key.pem"), #支付宝的公钥，验证支付宝回传消息使用，不是总价的公钥
		sign_type="RSA2", #RSA或者ＲＳＡ2
		debug=True #默认Ｆａｌｓｅ
	)
	while True:
		#进行支付结果查询
		result = alipay.api_alipay_trade_page_pay(order_id)
